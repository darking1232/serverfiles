	...
	switch (skill_id) {	//Calc base damage according to skill
		...
		case ML_SPIRALPIERCE:
			if (sd) {
				...
				ATK_ADDRATE(wd->damage, wd->damage2, 100*skill_lv); //Skill modifier applies to weight only.
			} else {
				wd->damage = battle_calc_base_damage(src, sstatus, &sstatus->rhw, sc, tstatus->size, 0); //Monsters have no weight and use ATK instead
			}
			...
			switch (tstatus->size) { //Size-fix. Is this modified by weapon perfection?
				case SZ_SMALL: //Small: 125%
					ATK_RATE(wd->damage, wd->damage2, 250);
					break;
				//case SZ_MEDIUM: //Medium: 100%
				case SZ_BIG: //Large: 75%
					ATK_RATE(wd->damage, wd->damage2, 150);
					break;
			}
#endif
			break;
...
#endif
			skillratio += 100*(7 + min(sstatus->sp,4500)/10);
			skillratio = min(400000,skillratio); //We stop at roughly 50k SP for overflow protection
			break;
...
#else
			ATK_ADD(wd->damage, wd->damage2, 80 * sc->data[SC_AURABLADE]->val1);
#endif
		}
	}
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag)
{
	...
	switch (skill_id) {
		...
		case HT_LANDMINE:
		case MA_LANDMINE:
			md.damage = skill_lv * (sstatus->dex + 100) * (120 + sstatus->int_) / 100;
			break;
		case HT_BLASTMINE:
			md.damage = skill_lv * (sstatus->dex / 2 + 70) * (100 + sstatus->int_) / 100;
			break;
		case HT_CLAYMORETRAP:
			md.damage = skill_lv * (sstatus->dex / 2 + 100) * (100 + sstatus->int_) / 100;
			break;
#endif
		case HT_BLITZBEAT:
		case SN_FALCONASSAULT:
			{
				...
				if (skill_id == SN_FALCONASSAULT) {
					...
					DAMAGE_DIV_FIX2(md.damage, skill_get_num(HT_BLITZBEAT, 15));
					//Falcon Assault Modifier
					md.damage = md.damage * (150 + 140 * skill_lv) / 100;
				}
			}
			break;
#ifndef RENEWAL
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "prevent_warp",                     &battle_config.prevent_warp,                  0,  0,      INT_MAX,          },
