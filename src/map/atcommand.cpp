	...
	if (pl_sd->vender_id ) {	 // check if player is vending
		for (j = 0; j < pl_sd->vend_num; j++) {
			...
			if(item_data->type == IT_ARMOR)
				snprintf(atcmd_output, CHAT_SIZE_MAX, "+%d %s[%d] | Price %u | Amount %d | %s[%d,%d] | ShopName %s",pl_sd->cart.u.items_cart[pl_sd->vending[j].index].refine
					,item_data->jname
				...
				snprintf(atcmd_output, CHAT_SIZE_MAX, "+%d %s[%d,%d,%d,%d] | Price %u | Amount %d | %s[%d,%d] | ShopName %s",pl_sd->cart.u.items_cart[pl_sd->vending[j].index].refine
					,item_data->jname
					,pl_sd->cart.u.items_cart[pl_sd->vending[j].index].card[0]
				...
				snprintf(atcmd_output, CHAT_SIZE_MAX, "%s | Price %u | Amount %d | %s[%d,%d] | ShopName %s",item_data->jname
					,pl_sd->vending[j].value
					,pl_sd->vending[j].amount
					,mapindex_id2name(pl_sd->mapindex)
					,pl_sd->bl.x, pl_sd->bl.y
...
ACMD_FUNC(whobuy)
{
    ...
    {
        clif_displaymessage(fd, "Not found item with this item NAME or ID.");
        return -1;
    }
...
ACMD_FUNC(mapmove)
{
	...
	nullpo_retr(-1, sd);
	if (!battle_config.prevent_warp || sd->canlog_tick == 0 || DIFF_TICK(gettick(), sd->canlog_tick) > battle_config.prevent_warp) {
	...
	clif_displaymessage(fd, msg_txt(sd,0)); // Warped.
		...
		sprintf(atcmd_output, "Proceed to safe area first and please wait %d seconds before warping.", battle_config.prevent_warp/1000);
		clif_displaymessage(fd, atcmd_output); 
	}
...
ACMD_FUNC(go)
{
	...
	const struct {
		...
		{ MAP_turbo_room,    99, 111 }, // 30=Turbo Room
	};
...
ACMD_FUNC(refresh)
{
	if( DIFF_TICK( sd->request_delay_tick,gettick() ) > 0 ){
		clif_displaymessage(fd,"There is a 3 Seconds delay in using @refresh command");
		return 0;
	}else{
		sd->request_delay_tick = gettick() + 3000; 
		nullpo_retr(-1, sd);
		clif_refresh(sd);
		return 0;
	}
...
/*==========================================
 * no @duel in prontera map - EliteBlack
 * Note: You may change the map "prontera"
 * to your own main map 
 *------------------------------------------*/
	if(sd->bl.m == map_mapname2mapid("prontera")) {
		clif_displaymessage(fd, "You can not duel in this map.");
		return 0;
	}
	if(sd->bl.m == map_mapname2mapid("prt_in")) {
		clif_displaymessage(fd, "You can not duel in this map.");
		return 0;
	}
	if(sd->bl.m == map_mapname2mapid("turbo_room")) {
		clif_displaymessage(fd, "You can not duel in this map.");
		return 0;
	}
